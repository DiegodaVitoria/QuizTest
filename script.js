...
continue_btn.onclick = ()=>{
    info_box.classList.remove("activeInfo"); //ocultar caixa de informação
    quiz_box.classList.add("activeQuiz"); //mostrar caixa de informações quiz box
...
function showResult(){
    ...
    if (userScore > 3){ 
        ...
        let scoreTag = '<span>K-ralho! 🎉, você fez <p>'+ userScore +'</p> de <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;  
    }
    else if(userScore > 1){ 
        let scoreTag = '<span>É 😎, você fez <p>'+ userScore +'</p> de <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
    else{ 
        let scoreTag = '<span>Tu é burro ein 😐, Você fez apenas <p>'+ userScore +'</p> de <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
}
...
function queCounter(index){
    ...
    bottom_ques_counter.innerHTML = totalQueCounTag;  //aadicionando nova tag de span dentro do ques_counter inferior
}